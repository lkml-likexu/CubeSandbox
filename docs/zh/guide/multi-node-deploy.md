...
- **物理机或裸金属服务器**（不支持嵌套虚拟化）
- **x86_64** 或 **aarch64** 架构，**已启用 KVM**（`ls /dev/kvm`）
- **Docker** 已安装并运行
- 到控制节点的**网络连通性**（默认需访问 `CubeMaster` 的 `8089` 端口）

::: tip 集群内所有节点架构必须保持一致
单个 Cube Sandbox 集群目前不支持 x86_64 与 aarch64 节点混部 —— 发布包中的 guest 内核和 `vmlinux` 与架构强相关。如需同时支持两种架构，请分别构建并发布对应架构的发布包。在 aarch64 控制 / 计算节点上，还需在 `.env` 中覆盖依赖服务的容器镜像为 arm64 版本（详见[裸金属 / 物理机部署 — aarch64 注意事项](./bare-metal-deploy.md#aarch64arm64-注意事项)）。
:::
