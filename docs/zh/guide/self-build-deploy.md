...
- **物理机或裸金属服务器**（不支持嵌套虚拟化）
- CPU 架构：**x86_64**（主推），或 **aarch64**（初步支持，详见下方说明）
- **已启用 KVM** — 通过 `ls /dev/kvm` 验证
- 推荐配置：8 核以上 CPU、16 GB 以上内存

::: tip aarch64 / arm64 初步支持
除 x86_64 外，Cube Sandbox 现已对 aarch64 架构提供初步支持（同样需要 `/dev/kvm`）。在 arm64 宿主机上部署时，需要在 `.env` 中将基础依赖容器镜像覆写为 arm64 兼容版本。
完整的镜像覆写片段、已覆盖的测试机型，以及当前能力差异（例如 **aarch64 不支持 PVM，且暂无该方向的开发计划**），请参阅[裸金属 / 物理机部署 — aarch64 / arm64 初步支持](./bare-metal-deploy.md#aarch64--arm64-初步支持)。
:::
