...
::: tip 已经有支持 KVM 的服务器？
如果你已经有一台开启了 KVM 的 Linux 服务器（物理机、裸金属服务器，或一台暴露了 `/dev/kvm` 的 aarch64 主机），可以直接参阅[裸金属 / 物理机部署](./bare-metal-deploy.md)，跳过 PVM 安装步骤。
:::

::: tip aarch64（arm64）用户
本快速开始依赖的 PVM 目前**仅支持 x86_64**，且**没有**移植到 aarch64 的计划。如果你在 arm64 机器上（目前已覆盖测试的机型为 HUAWEI 鲲鹏 920），请改看[裸金属 / 物理机部署](./bare-metal-deploy.md) — aarch64 已具备初步支持，同样要求宿主机暴露 `/dev/kvm`。
:::

## 前置条件

- **x86_64** 架构的云服务器（普通云服务器即可，无需 `/dev/kvm`）。aarch64 目前**不**支持这条 PVM 路径，aarch64 主机请改走[裸金属 / 物理机部署](./bare-metal-deploy.md)。
- 有 **root 权限**
